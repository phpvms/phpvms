...
test.describe("skylight dashboard (Workspace SPA theme)", () => {
  test("renders the Workspace board from the DTO, customizes, no console errors", async ({
    page,
  }) => {
    const errors: string[] = [];
  ...
  test("navigates dashboard → profile as an SPA visit (persistent layout, no full reload)", async ({
    page,
  }) => {
    const errors: string[] = [];
    page.on("console", (m) => m.type() === "error" && errors.push(m.text()));
    ...
    const survived = await page.evaluate(
      () => (window as Window & { __spa?: boolean }).__spa === true,
    );
