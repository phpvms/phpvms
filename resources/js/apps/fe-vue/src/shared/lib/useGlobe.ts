...
import { greatCircle, bearing, routeCenter, type LngLat } from "@/shared/lib/geo";
        ...
        layers: [
          {
            id: "ocean",
            type: "background",
            paint: { "background-color": token("--pv-globe-sea") },
          },
        ],
      },
      center,
    ...
    m.on("load", async () => {
      ...
      try {
        ...
        m.addLayer({
          ...
          type: "fill",
          source: "land",
          paint: { "fill-color": token("--pv-globe-land") },
        });
        m.addLayer({
          id: "land-coast",
          type: "line",
          source: "land",
          paint: { "line-color": token("--pv-globe-coast"), "line-width": 0.8 },
        });
      } catch {
      ...
      if (hasDest) {
        ...
        m.addSource("route", {
          ...
          data: {
            type: "Feature",
            properties: {},
            geometry: { type: "LineString", coordinates: greatCircle(route.from, to) },
          },
        });
        m.addLayer({
          id: "route",
