  ...
  return (
    <ModalBackdrop onClick={onClose}>
      <div
        ...
        class="flex max-h-[80vh] w-full max-w-lg flex-col rounded-lg bg-(--surface) shadow-xl"
        onClick={(e) => e.stopPropagation()}
      >
        <header class="border-b border-(--line) px-5 py-3">
          <h2 id="rf-help-title" class="text-base font-semibold text-(--ink)">
            {title}
          </h2>
        ...
        <div class="flex-1 overflow-auto px-5 py-3">
          <ul class="space-y-2">
            {items.map((item) => {
              ...
              const ring = isCurrent ? "border-primary-400 bg-(--surface-2)" : "border-(--line)";
              return (
        ...
        <footer class="flex justify-end gap-2 border-t border-(--line) px-5 py-3">
          <button type="button" class="fi-btn fi-color-gray" onClick={onClose}>
            Close
          </button>
        </footer>
      </div>
