...
const initials = computed(() =>
  name.value.split(' ').map((p) => p[0]).slice(0, 2).join('').toUpperCase(),
)
...
<template>
  ...
  <section class="pilot" aria-label="Pilot">
    ...
    <div class="who">
      <div class="line1">
        ...
        <span v-if="onLeave" class="leavechip">On leave</span>
      </div>
      <div class="line2">
        <span v-if="station" class="loc">
          <svg viewBox="0 0 24 24" class="pin" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 21s-7-6-7-11a7 7 0 0 1 14 0c0 5-7 11-7 11z"/><circle cx="12" cy="10" r="2.5"/></svg>
          {{ station }}
        </span>
      </div>
    </div>
  ...
  <div class="toolbar">
    ...
    <div class="tools">
      <div v-if="editing" ref="addRef" class="addwrap">
        <button type="button" class="btn" :disabled="!availableToAdd.length" @click="addOpen = !addOpen">
          <svg viewBox="0 0 24 24" class="i" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14M5 12h14"/></svg>
          Add widget
        </button>
        <div v-if="addOpen" class="menu">
          <div v-if="!availableToAdd.length" class="menu-empty">All widgets placed</div>
          <button v-for="w in availableToAdd" :key="w.id" type="button" class="menu-item" @click="add(w.id)">
            <PvIcon :name="w.icon" :size="15" class="i" />
            {{ w.title }}
          </button>
        </div>
      </div>
      <button v-if="editing" type="button" class="btn ghost" @click="resetLayout">Reset</button>
  ...
  <div class="board">
    ...
    <VueDraggable v-model="layout.sidebar" class="zone sidebar" v-bind="dragOpts" :disabled="!editing">
...
.name { font-size: 18px; font-weight: 600; color: var(--pv-ink); }
.rankchip {
  font-size: 11px; font-weight: 600; padding: 2px 8px; border-radius: var(--pv-radius-full);
  color: var(--pv-accent); background: var(--pv-accent-soft);
}
.leavechip {
  font-size: 11px; font-weight: 500; padding: 2px 8px; border-radius: var(--pv-radius-full);
  color: var(--pv-amber); background: color-mix(in srgb, var(--pv-amber) 12%, transparent);
}
.line2 { margin-top: 4px; }
.loc { display: inline-flex; align-items: center; gap: 4px; font-size: 12px; color: var(--pv-ink-dim); font-family: var(--pv-font-mono); }
.pin { width: 13px; height: 13px; }

.toolbar { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }
.h2 { font-size: 15px; font-weight: 600; color: var(--pv-ink); }
.tools { display: flex; align-items: center; gap: 8px; }
.btn {
  display: inline-flex; align-items: center; gap: 6px; height: 32px; padding: 0 12px;
  font-size: 12px; font-weight: 500; color: var(--pv-ink); background: var(--pv-panel);
  border: 1px solid var(--pv-line); border-radius: var(--pv-radius-md); cursor: pointer;
}
.btn:hover { background: var(--pv-hover); }
.btn:disabled { opacity: 0.5; cursor: not-allowed; }
.btn .i { width: 15px; height: 15px; }
.btn.primary { background: var(--pv-accent); border-color: var(--pv-accent); color: #fff; }
.btn.ghost { color: var(--pv-ink-dim); }
.addwrap { position: relative; }
.menu {
  position: absolute; right: 0; top: calc(100% + 6px); z-index: 40; min-width: 200px;
  background: var(--pv-panel); border: 1px solid var(--pv-line); border-radius: var(--pv-radius-lg);
...
.menu-item {
  ...
  font-size: 12px; color: var(--pv-ink); border-radius: var(--pv-radius-sm); cursor: pointer;
}
.menu-item:hover { background: var(--pv-hover); }
.menu-item .i { width: 15px; height: 15px; color: var(--pv-ink-dim); }
.menu-empty { font-size: 12px; color: var(--pv-ink-dim); padding: 8px; }

.board { display: grid; grid-template-columns: 1fr; gap: 16px; }
@media (min-width: 1100px) {
  .board { grid-template-columns: 1fr var(--pv-aside-width, 320px); align-items: start; }
}
.zone { display: grid; gap: 12px; min-height: 60px; }
.zone.grid { grid-template-columns: repeat(2, minmax(0, 1fr)); align-content: start; }
.zone.sidebar { grid-template-columns: 1fr; align-content: start; }
.cell { min-width: 0; }
:deep(.ws-ghost) { opacity: 0.4; }
</style>
