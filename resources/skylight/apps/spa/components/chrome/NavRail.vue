 ...
 */
interface Dest { label: string; href: string; spa: boolean; icon: string; badge?: string }
...
const dests: Dest[] = [
  ...
  { label: 'Logbook', href: '/pireps', spa: false, icon: 'notebook-text' },
...
<template>
  <div class="rail scroll-thin">
    ...
    <div class="mini">
      <div class="ma">
        <img v-if="user?.avatar && !imgFailed" :src="user.avatar" :alt="user?.name" @error="imgFailed = true" />
...
<style scoped>
.rail { height: 100%; display: flex; flex-direction: column; overflow-y: auto; }
.brand {
  height: var(--pv-header-height, 56px); display: flex; align-items: center; gap: 10px;
  padding: 0 14px; border-bottom: 1px solid var(--pv-line); flex-shrink: 0;
}
.logo { width: 26px; height: 26px; object-fit: contain; }
.bn { font-size: 13px; font-weight: 600; color: var(--pv-ink); }
.bs { font-size: 11px; letter-spacing: 0.05em; text-transform: uppercase; color: var(--pv-ink-faint); }

.nav { flex: 1; padding: 12px 8px; display: flex; flex-direction: column; gap: 2px; }
.sect { padding: 4px 8px 6px; }
.item {
  display: flex; align-items: center; gap: 10px; height: 34px; padding: 0 10px;
  border-radius: var(--pv-radius-md); color: var(--pv-ink-dim); font-size: 13px; font-weight: 500;
  text-decoration: none;
}
.item:hover { background: var(--pv-hover); color: var(--pv-ink); }
.item.active { background: var(--pv-accent-soft); color: var(--pv-accent); }
.item .i { width: 17px; height: 17px; flex-shrink: 0; }
.lbl { flex: 1; }

.mini {
  display: flex; align-items: center; gap: 10px; padding: 12px 14px;
  border-top: 1px solid var(--pv-line); flex-shrink: 0;
}
.ma {
  width: 32px; height: 32px; border-radius: var(--pv-radius-full); overflow: hidden; flex-shrink: 0;
...
.mn { font-size: 12px; font-weight: 500; color: var(--pv-ink); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.ms { color: var(--pv-green) !important; }
.dot { width: 7px; height: 7px; border-radius: 50%; background: var(--pv-green); flex-shrink: 0; }
</style>
