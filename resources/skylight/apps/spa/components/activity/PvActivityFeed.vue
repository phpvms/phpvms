...
<template>
  <div class="feed">
    <header class="head">
      ...
      <span class="lbl">{{ flyingNow === 1 ? "pilot" : "pilots" }} flying now</span>
    </header>

    <p v-if="state.status === 'loading'" class="muted">Loading activity…</p>
    <p v-else-if="state.status === 'error'" class="err">Couldn't load the activity feed.</p>
