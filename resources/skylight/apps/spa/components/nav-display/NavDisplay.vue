...
const trk = computed(() => (hasRoute.value ? `TRK ${String(Math.round(bearing(props.from, props.to as LngLat))).padStart(3, '0')}°` : 'TRK ---°'))
const dist = computed(() => (hasRoute.value ? `DIST ${Math.round(distanceNm(props.from, props.to as LngLat))}NM` : 'DIST ----NM'))
const ete = computed(() => {
...
<template>
  <div class="nd" role="img" :aria-label="`Nav display, route ${fromIcao} to ${toIcao ?? 'unknown'}`">
    <div class="nd-h">
      <span class="t">ND · MAPLIBRE · GLOBE</span>
      <span class="m">{{ fromIcao }} <template v-if="toIcao">▸ {{ toIcao }}</template></span>
    </div>
...
.readout {
  ...
  color: var(--pv-ink);
  background: linear-gradient(transparent, color-mix(in srgb, var(--pv-panel) 85%, transparent) 60%);
  pointer-events: none;
}
.readout .mag {
  color: var(--pv-accent);
