...
describe('WeatherWidget', () => {
  it('shows loading immediately, then success data', async () => {
    ...
    resolve({ ok: true, json: async () => ({ icao: 'KJFK', metar: 'KJFK 12345Z', conditions: 'CLR', temperature: '15C', wind: '270/10', taf: null }) })
    await flushPromises()
  ...
  it('renders a fail-visible error when the API returns an error payload', async () => {
    mockFetch(async () => ({ ok: false, status: 502, json: async () => ({ error: true, message: 'provider down', icao: 'KJFK' }) }))
    const w = mount(WeatherWidget, { props: { icao: 'KJFK' } })
    await flushPromises()
