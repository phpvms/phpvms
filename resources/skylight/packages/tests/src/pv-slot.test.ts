...
import { RegistryKey, ResolverKey, PagePropsKey } from '@/composables/usePvSlot'
...
describe('PvSlot', () => {
  it('renders registered components in ascending order with resolved @-ref props', () => {
    const registry: SlotEntry[] = [
      ...
      { slot: 'dashboard.sidebar', component: 'Widget', order: 10, props: { icao: '@currentAirport', tag: 'first' } },
    ]
  ...
  it('lets context override a page-prop of the same name', () => {
    const registry: SlotEntry[] = [
      { slot: 'dashboard.sidebar', component: 'Widget', order: 1, props: { icao: '@currentAirport' } },
    ]
    const w = mountSlot(
      registry,
      { Widget },
