...
/**
 * "The user has at least / at most / exactly N accepted PIREPs matching these
 * rules."
 *
 ...
 */
class PirepCountOperator extends PirepOperator
{
    ...
    public function applyToBaseQuery(Builder $query): Builder
    {
        ...
        // Security: settings arrive from a stored tree and can be tampered with.
        // A non-numeric count, an unoffered comparison, or a triggering-PIREP
        // scope with no PIREP bound applies nothing rather than reaching SQL.
        if ($count === null || $comparison === null || !$this->canApply()) {
            return $query;
        }

        return $query->has(
            PirepConstraint::RELATIONSHIP,
            $comparison,
            (int) $count,
            'and',
