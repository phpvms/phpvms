...
5. Do not define an on-time formula without an authoritative product rule and reliable schedule snapshot.
...
| On-time percentage                           | `onTimePercentage`                 | nullable until an authoritative schedule and tolerance rule exists                               |
...
### D4. Keep the summary presentation focused
...
### D5. Use one shared awards component
...
### D6. Preserve the theme and extension contracts
...
### D7. Keep persistent chrome separate
...
2. On-time percentage has no verified source. Mitigation: keep the field nullable and show `—` until the open question is resolved.
...
1. What makes a flight on time: departure or arrival, what tolerance applies, and where is the immutable scheduled timestamp stored? Until this is answered, `onTimePercentage` remains null.
